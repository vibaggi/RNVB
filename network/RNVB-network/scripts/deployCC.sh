...
if [ "$CC_RUNTIME_LANGUAGE" = "go" -o "$CC_RUNTIME_LANGUAGE" = "golang" ] ; then
	CC_RUNTIME_LANGUAGE=golang
	CC_SRC_PATH=$URL_CHAINCODE

	echo Vendoring Go dependencies ...
	pushd ../chaincode/rnvb/go
	...
	echo Supported chaincode languages are: go, javascript, java, typescript
	exit 1
fi
...
packageChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode package rnvb.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label rnvb_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode install rnvb.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/rnvb_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
approveForMyOrg() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode approveformyorg -o localhost:7050 --channelID $CHANNEL_NAME --name rnvb --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} --waitForEvent >&log.txt
    set +x
  else
    set -x
    peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.rnvb.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name rnvb --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
    set +x
  fi
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name rnvb --version ${VERSION} --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --channelID $CHANNEL_NAME --name rnvb $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.rnvb.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name rnvb $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name rnvb >&log.txt
    res=$?
    set +x
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n rnvb -c '{"Args":["queryAllCars"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
...
checkCommitReadiness 1 "\"brasil-governofederal-msp\": true" "\"sus-unidadesaude-msp\": false"
checkCommitReadiness 2 "\"brasil-governofederal-msp\": true" "\"sus-unidadesaude-msp\": false"
...
checkCommitReadiness 1 "\"brasil-governofederal-msp\": true" "\"sus-unidadesaude-msp\": true"
checkCommitReadiness 2 "\"brasil-governofederal-msp\": true" "\"sus-unidadesaude-msp\": true"
